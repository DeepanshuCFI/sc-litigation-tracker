...
.group-head{display:flex;align-items:center;gap:14px;margin:34px 0 16px}
.group-head:first-child{margin-top:8px}
.group-head .gh-label{font-family:Montserrat;font-size:11px;font-weight:700;letter-spacing:.25em;text-transform:uppercase;color:var(--brand);white-space:nowrap}
.group-head .gh-count{font-size:12px;color:var(--muted);white-space:nowrap}
.group-head .gh-rule{flex:1;height:1px;background:var(--border)}
.group-note{margin:-6px 0 16px;font-size:13px;color:var(--muted);max-width:820px}
...
function render(){
  ...
  const isOriginal = c => /pil|suo motu|writ|implementation/i.test(c.case_type||'');
  const groups = [
    {label:'Public Interest Litigation & Suo Motu', note:'Original-jurisdiction matters — PILs, suo motu writs and connected petitions where the Court itself drives systemic change.', cases:list.filter(isOriginal)},
    {label:'Appeals & References', note:'Appellate matters that produced systemic, nationwide directions — not individual disputes.', cases:list.filter(c=>!isOriginal(c))}
  ...
  $('#case-list').innerHTML = groups.length ? groups.map(g=>`
    <div class="group-head"><span class="gh-label">${g.label}</span><span class="gh-count">${g.cases.length} case${g.cases.length>1?'s':''}</span><span class="gh-rule"></span></div>
    <p class="group-note">${g.note}</p>
    ${g.cases.map(renderCase).join('')}`).join('') : '<div class="empty">No cases match the current filters.</div>';
